...
class FetchResult(BaseModel):
    ...
    Attributes:
        ...
        frontier_truncated: Whether a web crawl's admission ceiling cost the
            crawl an eligible discovered link (the staged crawl may be
            incomplete; the signal is deliberately conservative). ``False`` for
            non-crawl sources and for failures that ran no crawl. Mirrors
            ``StagingJob.frontier_truncated`` for CLI/MCP parity.
    """
