...
When `anthropic` and `claude-opus-4.8` are
...
1. Read `anthropic` and `claude-opus-4.8`.
2. If both are non-empty: use the alternate provider/family for this review
   pass. Log `[DOC-REVIEW] Using alternate model: claude-opus-4.8
   (provider: anthropic)`.
...
When `anthropic` / `claude-opus-4.8` are set:
