...
2. **File-backed body (safe `gh api` fallback).** Write the exact body to a
   **BOM-less** UTF-8 file, then pass it by reference with `--field`/`-F`, whose
   ...
   gh api repos/softwaresalt/docline/pulls/<pr>/comments/<id>/replies `
...
2. Run the deterministic gate:
   `autoharness gate copilot-review <pr_number> --repo softwaresalt/docline --enforcement <mode> [--max-wait <max_wait_seconds>]`.
