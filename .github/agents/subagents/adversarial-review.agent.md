...
alt_review_provider: "anthropic"
alt_review_family: "claude-opus-4.8"
...
Alternate model provider support (`anthropic` / `claude-opus-4.8`)
...
* `alt_provider`: (Optional) Alternate model provider name (e.g., `google`).
  Overrides `anthropic` for this invocation. When set, one reviewer
  slot is assigned to the alternate provider.
* `alt_family`: (Optional) Alternate model family (e.g., `gemini-2.5-flash`).
  Overrides `claude-opus-4.8` for this invocation. Paired with `alt_provider`.
...
4. Apply anchor and alternate model provider assignments:
   ...
   * Read `anthropic` and `claude-opus-4.8` (or `alt_provider`
...
| Reviewer-B | Tier 2 (standard) | `claude-sonnet-5` | `claude-opus-4.8` via `anthropic` |
...
`anthropic` is non-empty, Reviewer-B is routed to the alternate
...
* When `anthropic` is set, at least one non-anchor reviewer must use the alternate
